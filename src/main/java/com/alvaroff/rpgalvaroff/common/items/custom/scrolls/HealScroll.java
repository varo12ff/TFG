...
import com.alvaroff.rpgalvaroff.capabilities.playerSkills.HealSelfSkill;
...
public class HealScroll extends Item {
    public HealScroll(Properties p_41383_) {
        super(p_41383_);
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand){
        ...
        if(!world.isClientSide()){
            ...
            if(!PlayerUtils.isSkillLearned(skills, heal.getId())) {
                player.getCapability(PlayerStatsProvider.PLAYER_STATS).orElse(new PlayerStats()).addTotalSkills(heal.getId());
                itemStack.shrink(1);
            }
            else
